...
module Backtesting_engine_functor (S : S) = struct
  ...
  let process_order (oc : out_channel) (_ : ('local_config, 'local_state) Chronos_core.Strategy.t ref) (order : Chronos_core.Order.t) (_ : S.event) : unit =
    let trade = Trade.of_order ~fee_rate_one_side:0.001 order in
    let line = Trade.to_csv_line trade in
